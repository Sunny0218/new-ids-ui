...
export class JanusStreamingService {
  ...
  //初始化Janus
  janusInit(selectedStream:any) {
    Janus.init({
      debug:"all",
      dependencies: Janus.useDefaultDependencies({ adapter }),
      callback: () => {
        this.connectServer(this.server,selectedStream)
      }
    })
  ...
  //创建janus实例
  connectServer(server:any,selectedStream:any) {
    this.janus = new Janus({
      server,
      success: () => {
        this.attachPlugin(selectedStream);
      },
      error: (error:any) => {
  ...
  //连接到streaming插件
  attachPlugin(selectedStream:any) {
